...
class TestOne(db.Model):
    ...
    test = db.relationship('TestTwo', backref='test', lazy=True)


class TestTwo(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(50))
    test_id = db.Column(db.Integer, db.ForeignKey('test_one.id'))
...
#     parent = db.relationship('Category', backref='subcategories', remote_side='Category.id')
#
#     def __repr__(self):
#         return self.name
#
