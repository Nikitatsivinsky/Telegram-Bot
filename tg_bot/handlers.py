        ...
        match self.callback_data['route']:
            case '/items':
                if user_phone_number:
                    user_in_bd = Profile.query.filter_by(telephone=user_phone_number).first()
                    orders_list = Order.query.filter(Order.user_id == int(user_in_bd.id)).all()
                    print(user_in_bd.user, user_in_bd.login)
